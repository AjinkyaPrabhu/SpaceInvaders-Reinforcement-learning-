...
INPUT_DICT = {0:pygame.K_UP,1:pygame.K_DOWN,3:pygame.K_LEFT,4:pygame.K_RIGHT}
pygame.init()
...
def GrabGameImage():
    image_of_game = pygame.surfarray.array3d(screen)
    image_of_game = image_of_game.swapaxes(0,1)
    image_of_game = cv2.cvtColor(image_of_game,cv2.COLOR_BGR2GRAY)
    cv2.imshow('game_frame',image_of_game)
    return image_of_game
...
# GAME LOOP
    writer = tf.summary.FileWriter('./logs_rein')
    with tf.Session() as sess:
        writer.add_graph(graph=sess.graph)
        tf.summary.scalar("TOTAL LOSS",model.loss)
        merged_summary = tf.summary.merge_all()
        sess.run(tf.global_variables_initializer())


        for number_of_generation in range(100000):
            score = 0
            #Snake initialization
            mySnake = Snake(SCREEN_WIDTH/2,SCREEN_HEIGHT/2)
            mySnake.setDirection(KEY["UP"])
            ...
            while(endgame!=1):
                ...
                image_of_game = GrabGameImage()
                im_pl = image_of_game.reshape((1,480000))
                obs_data.append(im_pl)

                action = sess.run(model.sample_op,feed_dict={model.observation_placeholder:im_pl})

                print(action)
                # nn_out = sess.run(model.prediction,feed_dict={model.observation_placeholder:im_pl})
                ...
                #Input
                keyPress = INPUT_DICT[action[0,0]] #make input from network
                # keyPress = getKey()
                action_data.append(one_hot(action[0,0]))
                if keyPress == "exit":
                    endgame = 1
            ...
            print(np.array(obs_data))
            _,s = sess.run([model.train_op,merged_summary],feed_dict={model.observation_placeholder:np.array(obs_data).reshape((-1,480000)),model.action_placeholder:np.array(action_data),model.rewards_placeholder:np.array(reward_data)})
            writer.add_summary(s,number_of_generation)
            del obs_data,action_data,reward_data
